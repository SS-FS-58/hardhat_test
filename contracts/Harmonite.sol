...
// File: contracts/interfaces/IHMNTToken.sol


pragma solidity ^0.8.11;


interface IHMNTToken is IBEP20 {
...
contract VaultDistributor is IVaultDistributor {
    ...
    IHMNTToken eths;          
    IBEP20 WBNB = IBEP20(0x49D5c2BdFfac6CE2BFdB6640F4F80f226bc10bAB);
    address WONE = 0xB31f66AA3C1e785363F0875A1B74E27b85FD66c7;
    IDEXRouter router;
    ...
    function deposit() external payable override onlyToken {
        ...
        router.swapExactONEForTokensSupportingFeeOnTransferTokens{value: msg.value}(
            0,
            path,
            address(this),
            block.timestamp
        ...
        uint256 amount = WBNB.balanceOf(address(this)) - balanceBefore;
    ...
    // 0 is claim as BNB, 1 is compound to vault, 2 is claim as HMNT
...
// File: contracts/HarmonitesRewardManager.sol
...
contract HarmonitesRewardManager is Auth {

    uint256 public HMNTDecimals = 10 ** 18;
    ...
    mapping(uint256 => HarmoniteNode) nodeMapping;
    ...
    constructor(address _ethsTokenAddress) Auth(msg.sender) {
        ...
        ethsToken = IHMNTToken(_ethsTokenAddress);
    }
    ...
    function mintNode(string memory _name, uint256 _amount, uint256 _nodeType) public {
        ...
        HarmoniteNode memory nodeToCreate;

        if(_nodeType == 0) {
            require(_amount >= minMaxPebbles[0]*HMNTDecimals && _amount <= minMaxPebbles[1]*HMNTDecimals, "Pebble amount outside of valid range");
            nodeToCreate = HarmoniteNode({
                name: _name,
                id: nodeID,
        ...
        if(_nodeType == 1) {
            require(_amount >= minMaxShards[0]*HMNTDecimals && _amount <= minMaxShards[1]*HMNTDecimals, "Shard amount outside of valid range");
            nodeToCreate = HarmoniteNode({
                name: _name,
                id: nodeID,
                lastInteract: block.timestamp,
            ...
            require(_amount >= minMaxStones[0]*HMNTDecimals && _amount <= minMaxStones[1]*HMNTDecimals, "Stone amount outside of valid range");
            nodeToCreate = HarmoniteNode({
                name: _name,
                id: nodeID,
                lastInteract: block.timestamp,
            ...
            require(_amount >= minMaxCrystals[0]*HMNTDecimals && _amount <= minMaxCrystals[1]*HMNTDecimals, "Crystal amount outside of valid range");
            nodeToCreate = HarmoniteNode({
                name: _name,
                id: nodeID,
                lastInteract: block.timestamp,
    ...
    function compoundAllAvailableHarmoniteReward() external {
    ...
    function compoundHarmoniteReward(uint256 _id) external {
        HarmoniteNode memory etherstoneNode = getHarmoniteNodeById(_id);
    ...
    function claimAllAvailableHarmoniteReward() external {
        uint256 numOwnedNodes = accountNodes[msg.sender].length;
        require(numOwnedNodes > 0, "Must own nodes to claim rewards");
    ...
    function claimHarmoniteReward(uint256 _id) external {
    ...
    function calculateRewards(uint256 _id) public view returns (uint256) {
        ...
        return ((((((etherstoneNode.lockedHMNT
                                  ...
                                  * tierMultipliers[etherstoneNode.tier]) / tierMultipliersDenominator)
                                  + (etherstoneNode.lockedHMNT * baseNodeDailyInterest[etherstoneNode.nodeType]) / nodeDailyInterestDenominator)
    ...
    function getHarmoniteNodeById(uint256 _id) public view returns (HarmoniteNode memory){
        return nodeMapping[_id];
    }

    function getOwnedNodes(address _address) public view returns (HarmoniteNode[] memory) {
        uint256[] memory ownedNodesIDs = accountNodes[_address];
        HarmoniteNode[] memory ownedNodes = new HarmoniteNode[](ownedNodesIDs.length);
    ...
    function convertPresaleCrystalToReal(string memory _name) external {
        ...
        nodeMapping[nodeID] = HarmoniteNode({
            name: _name,
            id: nodeID,
            lastInteract: block.timestamp,
            lockedHMNT: 1000*10**18,
            nodeType: 3,
            tier: 0,
            timesCompounded: 0,
            owner: msg.sender
...
// File: contracts/HMNTToken.sol


pragma solidity ^0.8.11;

contract HMNTToken is IBEP20, Auth {

    address DEAD = 0x000000000000000000000000000000000000dEaD;

    HarmonitesRewardManager public rewardManager;
    ...
    string constant _name = 'Harmonites';
    string constant _symbol = 'HMNT';
    ...
    constructor() Auth(msg.sender) {
        ...
        pair = IDEXFactory(router.factory()).createPair(WONE, address(this));
    ...
    function setHarmonitesRewardManager(address _rewardManager) external onlyOwner {
        ...
        rewardManager = HarmonitesRewardManager(_rewardManager);
    ...
    function swapBack() internal swapping {
        ...
        router.swapExactTokensForONESupportingFeeOnTransferTokens(
            amountToSwap,
            0,
            path,
            address(this),
        ...
        uint256 amountONE = address(this).balance - balanceBefore;

        uint256 amountONELiquidity = (amountONE * liquidityFee) / (totalFee * 2);
        uint256 amountONEReflection = (amountONE * reflectionFee) / totalFee;
        uint256 amountONEMarketing = (amountONE * marketingFee) / totalFee;
        uint256 amountONETreasury = (amountONE * treasuryFee) / totalFee;

        if(amountONEReflection > 0) {
            distributor.deposit{ value: amountONEReflection }();
        }
        payable(marketingFeeReceiver).call{ value: amountONEMarketing }('');
        payable(treasuryFeeReceiver).call{ value: amountONETreasury }('');

        if (amountToLiquify > 0) {
            router.addLiquidityONE{value: amountONELiquidity}(
                address(this),
                amountToLiquify,
                0,
                0,
            ...
            emit AutoLiquify(amountONELiquidity, amountToLiquify);
        }
    }
    ...
    event AutoLiquify(uint256 amountONE, uint256 amountBOG);
